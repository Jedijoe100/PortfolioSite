    ...
    .get('/', async function (req, res, next) {
        ...
        for (const element of skills){
            let evidence = await fetchAll(db, 
                "SELECT Projects.ID, title, 'Projects' AS type FROM Projects INNER JOIN Evidence ON Evidence.Evidence_ID=Projects.ID WHERE Evidence.skill_ID = $id AND Evidence.Evidence_Table = 'Projects'"+
                "UNION SELECT Education.ID, topic AS title, 'Education' AS type FROM Education INNER JOIN Evidence ON Evidence.Evidence_ID=Education.ID WHERE Evidence.skill_ID = $id AND Evidence.Evidence_Table = 'Education'"+
                "UNION SELECT Experience.ID, title, 'Experience' AS type FROM Experience INNER JOIN Evidence ON Evidence.Evidence_ID=Experience.ID WHERE Evidence.skill_ID = $id AND Evidence.Evidence_Table = 'Experience'" +
                "UNION SELECT ScolarshipsAwardsVolunteering.ID, title, 'Other' AS type FROM ScolarshipsAwardsVolunteering INNER JOIN Evidence ON Evidence.Evidence_ID=ScolarshipsAwardsVolunteering.ID WHERE Evidence.skill_ID = $id AND Evidence.Evidence_Table = 'ScolarshipsAwardsVolunteering';",
                {$id:element.ID})
